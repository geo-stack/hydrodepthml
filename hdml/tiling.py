...
def filter_tiles(
    ...
    """
    Filter spatial tiles to retain only those that intersect given features.

    Reprojects features to match the tile grid if necessary, and selects
    tiles whose geometry intersects at least one feature. This enables
    efficient focus on relevant tiles for analysis, minimizing computations
    on empty or irrelevant tiles.

    Parameters
    ----------
    features_gdf : gpd.GeoDataFrame
        GeoDataFrame containing spatial features of any geometry type
        (e. g., Point, LineString, Polygon). Will be reprojected to match
        the tiles' CRS if needed.
    tiles_gdf : gpd.GeoDataFrame
