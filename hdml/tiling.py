...
def filter_tiles(features_path: Path, tiles_gdf: gpd.GeoDataFrame) -> dict:
    """
    Filter spatial tiles to retain only those that intersect features from
    a given file.

    Loads a vector file containing spatial features of any geometry type
    (e.g., points, lines, polygons), reprojects them to match the tile grid
    if necessary, and selects tiles whose geometry intersects at least one
    feature.

    Parameters
    ----------
    features_path : Path
        Path to a GeoPackage, GeoJSON, or other vector data file containing
        spatial features. These can be of any geometry type (Point, LineString,
        Polygon, etc.). The function will reproject them to match the tiles'
        CRS if needed.
    ...
    -------
    gpd.GeoDataFrame
        Filtered GeoDataFrame containing only tiles that intersect at least
        one feature in the input file. The index and structure are preserved
        as in the input 'tiles_gdf'
    ...
    # Reproject if needed.
    if features_gdf.crs != tiles_gdf.crs:
        print(f"Reprojecting features from {features_gdf.crs} "
              f"to {tiles_gdf.crs}...")
        features_gdf = features_gdf.to_crs(tiles_gdf.crs)

    # # Spatial join to retain intersecting tiles.
    joined = gpd.sjoin(
        tiles_gdf,
        features_gdf,
        how='inner',
    ...
    print(f"Filtered from {len(tiles_gdf)} to {len(filtered_tiles)} tiles "
          f"containing intersecting features.")
